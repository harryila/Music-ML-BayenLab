...
REPO="$PWD"
MAN="$REPO/data/pairs_classical_big_manifest.csv"
OUT="$REPO/MIDI2ScoreTransformer/checkpoints/ssl_bigc"
LOG="/root/ssl_bigc.log"
...
setsid nohup "$REPO/venv311/bin/python" -u \
  ...
  --stage ssl_bigc --dataset-type ssl --real-fraction 0.5 \
...
echo "ssl_bigc launched (pid $!) -> $LOG"
