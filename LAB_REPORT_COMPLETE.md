...
- **The pipeline works and is fully reproducible.** It has two blocks: **(1) audio→MIDI** (transcription) and **(2) MIDI→score** (notation). On clean, in-distribution piano it produces excellent scores — **end-to-end MeanER ≈ 2.75 on clean Chopin** — i.e. it **matches commercial quality on easy repertoire**, which makes sense because the Block-2 score model we run is the *same open released checkpoint Songscription is built on*. (The apples-to-apples comparison is the **11.18 full-test reproduction** below; 2.75 is the easy end of the difficulty range, not a like-for-like win over the product.)
...
- **The headline research result (this session):** across a broad experimental matrix, **no lever we tried achieved correct-*rate* triplet onset placement** — not data reshaping, self-training, a new tuplet-aware representation, loss-weighting, nor warm-start/from-scratch training. (Reshaping the corpus *over*-shot to **19.9%** placement but regressed MUSTER; everything else under-shot or collapsed near 0%.) We matched the released model's **validation loss** and came **within ~1 MUSTER point**, but never reproduced its correct-rate placement. **The leading, best-supported explanation is the training *data*: our corpus is ~1.7% tuplets — far poorer in the exact signal that's missing — whereas the released model's corpus/schedule are unpublished. We did *not* prove this is impossible: the decisive experiment (a tuplet-aware representation trained *to convergence* on tuplet-rich paired data) was left undertrained — so this is "not recovered within our compute/data budget," not a proof of an irreproducible secret recipe.**
...
We reproduced the released model's **architecture** and its **validation loss**, and came **within ~1 MUSTER point** — but never reproduced its correct-*rate* triplet placement under any lever we tried (corpus reshaping *over*-shot to **19.9%** with a MUSTER regression; everything else under-shot or collapsed near 0%). Importantly, the most promising configuration — a tuplet-aware representation (B2) trained **to convergence** on tuplet-rich paired data — was **not** completed: that from-scratch run landed **undertrained** (val 1.3 vs the 0.51 baseline).

**The leading, best-supported explanation is the training *data*.** Our corpus is ~1.7% tuplets — poor in exactly the signal that's missing — while the released model's corpus, masking/beat-alignment schedule, and music21 fork are unpublished. We did **not** prove placement is unrecoverable; we showed it is **not recovered within our compute/data budget**, and that the one experiment that would settle it (a tuplet-aware representation trained *to convergence* on tuplet-rich paired data) remains **open**. This is an honest, well-bounded characterization of a real failure mode — and it pins where the remaining accuracy most likely lives: **in the data.** (Cheap next step we have *not* done: email the paper's author — the recipe is unpublished, not unobtainable.)
...
2. **You do not need to *beat* the audio→score SOTA to build the data engine — you can *use* it.** The released model is the placement-capable engine. We showed you *can't cheaply rebuild* its placement within a semester's compute/data budget, which is precisely the argument for using it off-the-shelf as the engine and putting effort into the **training corpus** (the binding constraint) rather than re-deriving the recipe from scratch.
...
- **Product / pipeline:** ship the released model as the Block-2 engine (it has placement); my pipeline (hFT + released model + the eval harness) already **matches commercial quality on clean piano (2.75 end-to-end — running the same open score model Songscription is built on)**.
...
> *"I reproduced the published audio→score SOTA, built the full pipeline (which **matches commercial quality on clean piano** — the score brain is the open part), and traced the one remaining accuracy ceiling — triplet rhythmic notation — to the training **data** (a tuplet-poor corpus), not a tuning gap or an irreproducible recipe; that tells us to use the released model as a **data engine** and to own the shared **music-notation representation** the generative side also needs."*
...
- **This session's narrative:** `docs/sessions/MORNING_REPORT_AUTONOMOUS.md`, `docs/sessions/AUTONOMOUS_SESSION_LOG.md`, `docs/archive/LIT_TUPLET_FINDINGS.json`
